...
const ALL_COURTS = ["court1", "court2", "court3", "court4", "court5"] as const;
...
export default function LiveAllPage() {
  ...
  // Pick exactly 4 courts from ?courts=court1,court3,...; fallback to first 4.
  const selected = useMemo(() => {
    ...
    const list = raw
      ? raw
          .split(",")
          .map((s) => s.trim())
          .filter((s) => ALL_COURTS.includes(s as any))
      : ALL_COURTS.slice(0, 4);
    // ensure exactly 4 (pad from ALL_COURTS if needed)
    const withPad = [...new Set(list)];
    for (const c of ALL_COURTS) if (withPad.length < 4 && !withPad.includes(c)) withPad.push(c);
    return withPad.slice(0, 4) as typeof ALL_COURTS[number][];
  }, [params]);

  // Subscribe only to the 4 visible courts
    ...
    (async () => {
      ...
      selected.forEach((id) => {
        const unsub = onValue(ref(db, `/courts/${id}`), (snap) => {
          ...
          loaded += 1;
          if (loaded >= selected.length) setLoading(false);
        });
        unsubs.push(unsub);
      });
    })();
  ...
  return (
    <main style={{ minHeight:"100vh", background:"#212A31", color:"#E9EDF3", padding:"2rem 1.2rem" }}>
      ...
      {/* Brand header with spacing; Joy & Courts magenta, Division teal */}
      <div className="wrap">
        <h1
          className="title"
          style={{
        ...
        {loading ? (
          ...
          <section className="grid">
            {selected.map((id) => (
              <CourtCard key={id} courtId={id as typeof ALL_COURTS[number]} s={states[id] ?? DEFAULT} />
            ))}
          </section>
        )}
      </div>
...
/* Card component */
function CourtCard({ courtId, s }: { courtId: typeof ALL_COURTS[number]; s: ScoreState }) {
  const maxSets = useMemo(() => ((s?.meta?.bestOf ?? 3) === 5 ? 5 : 3), [s?.meta?.bestOf]);
  const title = (s?.meta?.name || "").trim() ? s.meta.name : `Court ${courtId.slice(-1)}`;
  ...
  return (
    <article className="card">
      <div className="header"><div className="court">{title}</div></div>
      <div className="rows"><Row side="p1" /><Row side="p2" /></div>
    </article>
  );
}
