...
function normalize(v: any): ScoreState {
  ...
  return {
    meta: { name: v?.meta?.name ?? "", bestOf: v?.meta?.bestOf === 5 ? 5 : 3 },
    players: {
    ...
    games: { p1: Number.isFinite(v?.games?.p1) ? v.games.p1 : 0, p2: Number.isFinite(v?.games?.p2) ? v.games.p2 : 0 },
    sets: { p1: Array.isArray(v?.sets?.p1) ? v.sets.p1 : [], p2: Array.isArray(v?.sets?.p2) ? v.sets.p2 : [] },
    tiebreak: !!v?.tiebreak,
    tb: { p1: Number.isFinite(v?.tb?.p1) ? v.tb.p1 : 0, p2: Number.isFinite(v?.tb?.p2) ? v.tb.p2 : 0 },
...
function sanitizeCourt(raw?: string | string[]): "court1"|"court2"|"court3"|"court4"|"court5" {
  ...
  return ok.has(s || "") ? (s as any) : "court1";
}

export default function ControllerPage({ searchParams }: PageProps) {
  const courtId = sanitizeCourt(searchParams?.court);
  const COURT_PATH = `/courts/${courtId}`;
  const NAME_PATH = `/courts/${courtId}/meta/name`;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      unsub2 = onValue(ref(db, NAME_PATH), (snap) => setExternalCourtName(typeof snap.val() === "string" ? snap.val() : ""));
    })();
    return () => { unsub1?.(); unsub2?.(); };
  }, [COURT_PATH, NAME_PATH]);
  ...
  function addPoint(side: Side, dir: 1 | -1) {
    ...
    if (n.tiebreak) {
      ...
      if ((a >= 7 || b >= 7) && Math.abs(a - b) >= 2) {
        ...
        n.games = { p1: 0, p2: 0 }; n.points = { p1: 0, p2: 0 };
  ...
  function resetPoints() {
    const n = clone();
    if (n.tiebreak) n.tb = { p1: 0, p2: 0 };
    else n.points = { p1: 0, p2: 0 };
    commit(n);
  }
  ...
  function renderRow(side: Side) {
    ...
    return (
      ...
      >
        ...
        <div className="scoreGrid" style={{ display: "grid", gridTemplateColumns: `repeat(${maxSets + 1}, 1fr)`, gap: "0.6em" }}>
          {setCells.map((v, i) => (
  ...
  return (
    <div className="pageWrap" style={{ background: "var(--c-ink)", minHeight: "100vh" }}>
      <style>{`
        ...
        .card{ background: var(--c-ink-2); color: #fff; border: 1px solid rgba(0,0,0,0.15);
               border-radius: 16px; padding: 1rem; box-shadow: 0 6px 20px rgba(0,0,0,0.25); }
        .headerBar{ display:flex; gap:1rem; padding:.5rem .75rem .9rem; border-bottom: 1px solid rgba(211,217,212,0.16); }
        ...
        .teamCard{ background: rgba(33,42,49,0.45); border: 1px solid rgba(211,217,212,0.12); border-radius: 12px; padding: 1rem; font-size: 1em; }
        .input{ width: 100%; background: var(--c-cloud); border: 1px solid var(--c-muted); color: var(--c-ink);
                border-radius: 10px; height: 2.6em; padding: 0 .9em; font-size: 1em; }
        .input::placeholder{ color: var(--c-muted); }
        .input:focus{ outline: 2px solid var(--c-primary); border-color: var(--c-primary); }
        .btn{ border: 1px solid transparent; background: var(--c-primary); color: #fff; border-radius: 12px; height: 2.8em;
              padding: 0 1.1em; font-weight: 700; font-size: 1em; transition: transform .06s ease, filter .12s ease; }
        .btn:hover{ filter: brightness(1.05); transform: translateY(-1px); }
        .btn:active{ transform: translateY(0); }
        .btn-lg{ height: 2.8em; }
        .btn-xl{ height: 3.0em; font-size: 1.15em; }
        ...
        .bestOfSelect{ background: var(--c-cloud); color: #0b1419; border:1px solid var(--c-muted); }
      `}</style>
      ...
      <div className="container" style={{ width: "min(1200px, 95vw)", paddingTop: 18, paddingBottom: 24 }}>
        <div className="card">
          <div className="headerBar" style={{ justifyContent: "space-between", alignItems: "end" }}>
            <div className="courtName">{externalCourtName || courtIdLabel(courtId)}</div>
            <div className="stack" style={{ alignItems: "end", gap: ".5rem" }}>
              <div style={{ display: "flex", gap: ".75rem", alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
                <select
                  ...
                  style={{ width: "10em", borderRadius: 9999, height: "2.4em", padding: "0 .9em" }}
                >
          ...
          <div className="teamsGrid">
            <div className="teamCard">
              <div className="stack" style={{ gap: ".75rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".75rem" }}>
                  <PlayerBlock s={s} kx="1a" onChange={updatePlayer} label="Player 1" />
                  <PlayerBlock s={s} kx="1b" onChange={updatePlayer} label="Player 2" />
                </div>
            ...
            <div className="teamCard">
              <div className="stack" style={{ gap: ".75rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".75rem" }}>
                  <PlayerBlock s={s} kx="2a" onChange={updatePlayer} label="Player 3" />
                  <PlayerBlock s={s} kx="2b" onChange={updatePlayer} label="Player 4" />
                </div>
                <div className="teamsGrid" style={{ gridTemplateColumns: "1fr 1fr" }}>
...
function PlayerBlock({ s, kx, onChange, label }:{
  s: ScoreState;
  kx: "1a"|"1b"|"2a"|"2b";
  onChange: (k:"1a"|"1b"|"2a"|"2b", f:"name"|"cc", v:string)=>void;
  label: string;
}) {
  return (
    <div className="stack" style={{ gap: ".4rem" }}>
      <label style={{ color: "var(--c-cloud)", fontSize: "1em" }}>{label}</label>
      <input
        className="input"
        placeholder="Enter Name"
        value={s.players[kx].name}
        onChange={(e) => onChange(kx, "name", e.target.value)}
        maxLength={30}
      />
      <select
        className="input"
        value={s.players[kx].cc}
        onChange={(e) => onChange(kx, "cc", e.target.value)}
      >
        {COUNTRIES.map(([f, n]) => (
...
function courtIdLabel(c: string) {
  return c === "court1" ? "Centre Court" : c.replace("court", "Court ");
}
