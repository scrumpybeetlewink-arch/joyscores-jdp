...
function ControllerInner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      offName = onValue(ref(db, NAME_PATH), (snap) => setExternalName(typeof snap.val() === "string" ? snap.val() : ""));
    })();
  ...
  function resetGame() { const n = clone(); const { p1:g1, p2:g2 } = n.games; if (g1>g2) n.games.p1=Math.max(0,g1-1); else if (g2>g1) n.games.p2=Math.max(0,g2-1); commit(n); }
  function newMatch() {
    commit({ ...defaultState, meta: { name: externalName, bestOf: (s.meta?.bestOf ?? 3) as BestOf }, players: defaultState.players, server: "p1", ts: Date.now() });
  }
  async function updatePlayer(key: "1a"|"1b"|"2a"|"2b", field: "name"|"cc", val: string) { const n=clone(); (n.players[key] as any)[field]=val; await commit(n); }
    ...
    return (
      <div className="row">
        ...
        <div className="grid" style={{ gridTemplateColumns: `repeat(${maxSets + 1},1fr)` }}>
          {cells.map((v, i) => <div key={i} className="box">{v}</div>)}
      ...
      <div className="container">
        <div className="card">
          <div className="head">
            ...
            <select aria-label="Best of" className="select" value={s.meta?.bestOf ?? 3} onChange={(e) => updateBestOf((Number(e.target.value) as 5|3) || 3)}>
          ...
          <div className="panelGrid">
            <div className="panel">
              ...
              <select className="input" style={{ marginTop: 8 }} value={s.players["1a"].cc} onChange={(e) => updatePlayer("1a", "cc", e.target.value)}>
            ...
            <div className="panel">
              ...
              <select className="input" style={{ marginTop: 8 }} value={s.players["1b"].cc} onChange={(e) => updatePlayer("1b", "cc", e.target.value)}>
                {COUNTRIES.map(([f, n]) => <option key={`${f}-${n}`} value={f}>{f} {n}</option>)}
              </select>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
