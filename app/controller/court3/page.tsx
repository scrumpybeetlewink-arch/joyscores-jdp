...
function useScore(courtId: string) {
  ...
  const write = (next: ScoreState) => set(ref(db, `courts/${courtId}/score`), next);

  const step = (p: Point): Point | "WIN" => (p === 0 ? 15 : p === 15 ? 30 : p === 30 ? 40 : "WIN");

  const increment = (side: Side) => {
    ...
    if (s.meta.goldenPoint && s.points.p1 === 40 && s.points.p2 === 40) {
      s.games[side] += 1; s.points = { p1: 0, p2: 0 }; s.last = { t: "game", side }; return write(s);
    }
  ...
  const undo = () => {
    ...
    if (last.t === "pt") {
      const side = last.side as Side; const p = s.points[side];
  ...
  const toggleGP = () => { const s: ScoreState = JSON.parse(JSON.stringify(state)); s.meta.goldenPoint = !s.meta.goldenPoint; return write(s); };

  return { state, actions: { increment, undo, swap, resetGames, resetMatch, toggleGP } };
}

export default function Page() {
  const { state, actions } = useScore(COURT_ID);
  const { increment, undo, swap, resetGames, resetMatch, toggleGP } = actions;
  ...
  return (
    <div style={{ background: "var(--bg)", color: "var(--text)", minHeight: "100vh", padding: 16 }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 12, background: "var(--panel)", borderRadius: 16 }}>
        <h1 style={{ margin: 0 }}>Controller • {state.meta.name} ({COURT_ID})</h1>
        <button onClick={toggleGP} style={{ padding: "8px 12px", background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 8, color: "var(--text)" }}>
      ...
      <main style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
        <section style={{ background: "var(--panel)", borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={() => increment("p1")} style={{ padding: "10px 16px", background: "var(--accent)", color: "var(--bg)", border: "none", borderRadius: 10 }}>+ P1 Point</button>
          </div>
        </section>
        ...
        <section style={{ background: "var(--panel)", borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={() => increment("p2")} style={{ padding: "10px 16px", background: "var(--accent)", color: "var(--bg)", border: "none", borderRadius: 10 }}>+ P2 Point</button>
          </div>
        </section>
      </main>
      ...
      <section style={{ background: "var(--panel)", borderRadius: 16, padding: 16, marginTop: 16, display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <button onClick={swap} style={{ padding: "10px 16px", background: "transparent", border: "1px solid var(--line)", color: "var(--text)", borderRadius: 10 }}>Swap Sides</button>
