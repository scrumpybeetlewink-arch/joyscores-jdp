...
export default function ControllerClient({ courtId }: { courtId: "court1"|"court2"|"court3"|"court4"|"court5" }) {
  ...
  useEffect(() => {
    ...
    const unsub = onValue(base, (snap) => {
      const val = snap.val();
      if (val) {
        // normalize some optional fields
        val.sets ||= { p1: [], p2: [] };
        val.tb ||= { p1: 0, p2: 0 };
      }
  ...
  function toggleGolden() {
    write({ meta: { ...state.meta, golden: !state.meta?.golden } as any });
  }

  function setServer(side: Side) {
    write({ server: side } as any);
  }

  function point(side: Side, delta: 1|-1) {
    const order: (Point)[] = [0, 15, 30, 40, "Ad"];
    const idx = order.indexOf(state.points[side] as Point);
    let next = idx;
    if (delta === 1) next = Math.min(idx + 1, order.length - 1);
    else next = Math.max(idx - 1, 0);
    write({ points: { ...state.points, [side]: order[next] } } as any);
  }
